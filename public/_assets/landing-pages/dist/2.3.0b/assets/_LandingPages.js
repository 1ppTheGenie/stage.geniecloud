...
const _LandingPages = (() => {
    ...
    if (layers && layers.listings) {
      ...
      layers.listings.forEach(l => {
        if (!isNaN(l.lat) && !isNaN(l.lng)) {
          ...
          const color = l.color || window.gHub.getCssVar(`--${l.state}`, document.body).trim();
          switch (l.iconType ?? "circle") {
            case "flag":
              size = [55 * scale, 38 * scale];
              anchor = [27 * scale, 38 * scale];
              datatURI = window.gHub.flagURI(color, window.gHub.currency(parseInt(l.state == "sold" ? l.salePrice : l.listPrice)));
              break;
            case "dot":
              size = [15, 15];
              anchor = [9, 9];
  ...
  window.gHub.getCssVar = (name, el) => getComputedStyle(el ?? document.body).getPropertyValue(name);
  ...
  document.addEventListener("trigger-update-lead", async e => {
    ...
    if (settings.leadId || settings.genieLeadId || window.gHub.leadId) {
      await updateLead({
        genieLeadId: settings.leadId || settings.genieLeadId || window.gHub.leadId,
        ...e.detail
      });
    }
  });
  ...
  window.gHub.addLead = async (note, data = null) => {
    ...
    let postedData = {
      ...
      propertyId: settings.propertyId ?? window.gHub.defaults.leadPropertyId ?? new URLSearchParams(window.location.search).get("propertyId") ?? new URLSearchParams(window.location.search).get("propertyID"),
    ...
    if (settings.leadId || settings.genieLeadId || window.gHub.leadId) {
      if (note !== VIEWED) {
        postedData.genieLeadId = settings.leadId || settings.genieLeadId || window.gHub.leadId;
      ...
      if (window.gHub.defaults.notePrompt && window.gHub.defaults.notePrompt !== "") {
  ...
  if (typeof window.gHub.galleryImages !== "undefined") {
    ...
    if (gallery) {
      ...
      const GalleryComponent = () => createComponent(Gallery, {
        get images() {
          return window.gHub.galleryImages;
        }
      });
