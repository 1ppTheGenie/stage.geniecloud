...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template name="ed-listings-table">
					...
					<xsl:for-each select="$listings">
						<xsl:if test="position() &lt;= 12">
							<tr>
								...
								<td>
									<xsl:value-of select="genie:format-date( number(@listedDate), '[M02]-D-[Y0001]' )" />
								</td>
								<td>
		...
		<style>
			...
			table.listings th {
    			padding: 1% 0 1%;
	 			border-bottom: 1px solid var(--theme-emphasis-color);
	 			margin-bottom:1%;
				}
			table.listings td {
    			border-bottom: none;
	...
	<xsl:template name="map-listings-table-header">
		...
		<style>
			...
			<xsl:value-of select="' #genie-wrapper[viewBox=&quot;0 0 555 555&quot;] .hide-small {display: none;}
		...
		<style>
			<xsl:value-of select="concat('
				:root {
					...
					--theme-sub-heading-color: var(--color-scheme); 
				}')" />
