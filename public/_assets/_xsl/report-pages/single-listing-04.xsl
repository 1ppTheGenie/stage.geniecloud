...
<xsl:import href="common.xsl" />
	...
	<xsl:template name="svg-body">
		...
		<g>
			...
			<text x="2.35%" y="6%" transform-origin="4% 4%" transform="rotate(-47)" fill="var(--theme-body-background)" class="align-center">
...
<xsl:value-of select="$singularPropertyType" />
			...
			<g style="transform:translateY(74%)">
				<svg width="35%" height="17.5%">
					<use x="17.6%" y="16%" width="10%" height="10%" fill="var(--theme-sub-heading-color)" style="transform: rotate(95deg) translate(-14.3%, -130.5%);">
						<xsl:attribute name="href" select="concat( //output/@siteUrl, '_assets/_img/icons.svg#phone-icon' )" />
					</use>
					<use x="23%" y="50%" fill="var(--theme-sub-heading-color)" width="10%" height="10%">
						<xsl:attribute name="href" select="concat( //output/@siteUrl, '_assets/_img/icons.svg#globe-icon' )" />
					</use>
					<use x="23%" y="65%" fill="var(--theme-sub-heading-color)" width="10%" height="10%">
						<xsl:attribute name="href" select="concat( //output/@siteUrl, '_assets/_img/icons.svg#email-icon' )" />
					</use>
					<image x="0" y="0" width="25%" height="100%" preserveAspectRatio="xMidYMid slice">
						<xsl:attribute name="href">
							<xsl:value-of select="//agent[1]/photo" />
						</xsl:attribute>
					</image>
					<text x="27%" y="7%" font-size="145%" fill="var(--theme-body-color)">
						<xsl:call-template name="editable">
							<xsl:with-param name="id" select="'agentname'" />
							<xsl:with-param name="default" select="//agent[1]/marketingName" />
						</xsl:call-template>
					</text>
					<text x="31%" y="32%" fill="var(--theme-sub-heading-color)" style="font-size:90%;">
						<xsl:call-template name="editable">
					...
					<text x="31%" y="47%" fill="var(--theme-sub-heading-color)" style="font-size:90%;">
						<xsl:call-template name="editable">
					...
					<text x="31%" y="63%" fill="var(--theme-sub-heading-color)" style="font-size:90%;">
						<xsl:call-template name="editable">
