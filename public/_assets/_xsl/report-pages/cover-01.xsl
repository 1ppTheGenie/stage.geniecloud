				...
				<xsl:if test="//output/@customerName!=''">
					<foreignObject x="20%" y="14%" height="10%" width="60%">
						<div style="display:flex;justify-content:center;align-items:center;">
							...
							<h2 contenteditable="true" class="upper" style="color: var(--theme-sub-heading-color);text-align: center;font-weight: 400; font-family:var(--theme-sub-heading-font)">Market Insider REPORT</h2>
...
<xsl:value-of select="//output/@customerName" />
				...
				<xsl:if test="//output/@customerName!=''">
					...
					<text x="50%" y="52.5%" class="narrow center" font-size="140%" style="fill:var(--theme-body-color);">
						<xsl:value-of select="genie:format-date( //output/@reportDate, '[FNn], [MNn] [D], [Y0001]' )" />
					</text>

					<xsl:if test="//agent[1]/marketingName!=''">
						<text x="50%" y="62.5%" class="medium center bold" style="fill:var(--theme-body-color);">
							<xsl:value-of select="concat( 'Courtesy of ', //agent[1]/marketingName )" />
						</text>
					</xsl:if>

					<image x="35%" y="70%" width="30%" height="12%" id="logo" preserveAspectRatio="xMidYMid meet">
				...
				<xsl:if test="//output/@customerName=''">
					...
					<text x="50%" y="44.5%" class="center" font-size="140%" style="fill:var(--theme-body-color);" font-family="var(--theme-body-font)">
						<xsl:value-of select="genie:format-date( //output/@reportDate, '[FNn], [MNn] [D], [Y0001]')" />
					</text>

					<xsl:if test="//agent[1]/marketingName!=''">
						<text x="50%" y="53.5%" class="bold center" style="fill:var(--theme-body-color);font-size:120%;" font-family="var(--theme-body-font)">
							<xsl:value-of select="concat( 'Courtesy of ', //agent[1]/marketingName )" />
						</text>
					</xsl:if>

					<image x="35%" y="62%" width="30%" height="12%" id="logo" preserveAspectRatio="xMidYMid meet">
						<xsl:attribute name="href">
							<xsl:value-of select="$companyLogo" />
						</xsl:attribute>
					</image>
