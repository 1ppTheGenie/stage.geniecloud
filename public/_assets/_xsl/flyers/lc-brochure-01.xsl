...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:genie="https://theGenie.ai/hub" version="3.0"
	...
	<xsl:template name="svg-body">
		...
		<g>
			<text x="15.5%" y="5.5%" class="center" font-size="115%" font-weight="300"
				font-family="var(--theme-sub-heading-font)" fill="var(--theme-heading-color)">
					...
					<p class="center upper"
						...
						<xsl:call-template name="editable">
							...
							<xsl:with-param name="default"
								select="concat( genie:indefinite-article( string(//area/name) ), ' ', //area/name, '&#160;Gem' )" />
						</xsl:call-template>
					</p>
    ...
    text-transform: uppercase;">
					...
					<div style="display:flex;justify-content:center;align-items:center;margin-top:25px;">
						<svg height="13" width="60" style="flex-grow:0; margin-right: 5%">
							<path stroke-linecap="butt" d="M0 5 l75 0" fill="none" stroke="var(--theme-heading-color)"
								stroke-width="6"></path>
						</svg>

						<h2 class="bold"
							style="font-family:var(--theme-sub-heading-font);color:var(--theme-sub-heading-color);font-size:43px;text-align:center;margin:0;">
			...
			<g style="transform:translate(4%, 23%)">
				<text x="0" y="0" class="bold"
					style="font-family:var(--theme-sub-heading-font);color:var(--theme-sub-heading-color);"
					font-size="280%">
					<xsl:choose>
						<xsl:when test="//single/soldDate!=''">
							<xsl:value-of
								select="concat( 'Sold For ', format-number( //single/salePrice, '$###,###') )" />
						</xsl:when>
						<xsl:otherwise>
				...
				<foreignObject y="3.5%" width="60%" height="15%" id="lineDescription">
					<p
						style="font-size:29px;font-family:var(--theme-body-font);color:var(--theme-body-color);margin:0;">
						<xsl:call-template name="editable">
			...
			<g style="transform: translate(88%, 15%);">
				...
				<text x="0%" y="1.5%" font-size="var(--theme-body-font)" fill="var(--theme-body-color)"
					style="font-size:65%" font-weight="500">
			...
			<g style="transform:translate(62%, 18%)">
				<svg width="88%" height="50%" x="-9%" y="5%">
					<use x="3" y="2.8%" width="57%" height="121" fill="var(--theme-heading-color)">
						<xsl:attribute name="href"
							select="concat( //output/@siteUrl, '_assets/_img/icons.svg#bed-icon' )" />
					</use>
				...
				<svg width="88%" height="50%" x="-9%" y="11%">
					<use x="3" y="2.8%" width="57%" height="121" fill="var(--theme-heading-color)">
						<xsl:attribute name="href"
							select="concat( //output/@siteUrl, '_assets/_img/icons.svg#bath-icon' )" />
					</use>
					<text x="39.8%" y="5%" class="bold" fill="var(--theme-heading-color)" style="font-size: 470%;">
						<xsl:call-template name="editable">
				...
				<svg width="88%" height="50%" x="-9%" y="18%">
					...
					<svg x="21%" y="2.8%" width="12%" height="12%" viewBox="-86 -66 512 512"
					...
					<xsl:if test="//single/parking/@spaces!='' and //single/parking/@garage!=''">
						<text x="39.8%" y="4.5%" class="bold" fill="var(--theme-heading-color)" font-size="470%">
							<xsl:call-template name="editable">
								<xsl:with-param name="id" select="'parkingspaces'" />
								<xsl:with-param name="default"
									select="//single/parking/@spaces+//single/parking/@garage" />
							</xsl:call-template>
						</text>
					</xsl:if>
				</svg>
					...
					<svg width="100%" height="100%">
						...
						<g style="transform:translate(24%, 0%)">
							...
							<text class="align-right bold"
								...
								x="23%" y="4%" font-size="250%">
								<xsl:call-template name="editable">
									<xsl:with-param name="id" select="'agentname'" />
									<xsl:with-param name="default" select="//agent[1]/marketingName" />
								</xsl:call-template>
							</text>
							<text class="align-right" style="font-family:var(--theme-body-font);" x="23%" y="6.8%"
								fill="var(--theme-sub-heading-color)" font-size="200%" font-weight="700">
								<xsl:call-template name="editable">
									<xsl:with-param name="id" select="'agentmobile'" />
									<xsl:with-param name="default" select="//agent[1]/mobile" />
								</xsl:call-template>
							</text>
							<text x="23%" y="10%" class="align-right" fill="var(--theme-sub-heading-color)"
								style="font-size:150%; font-family:var(--theme-body-font); dominant-baseline: middle;"
								data-max-width="25%">
								<xsl:call-template name="editable">
							...
							<text x="23%" y="12%" class="align-right" fill="var(--theme-sub-heading-color)"
								style="font-size:150%; font-family:var(--theme-body-font); dominant-baseline: middle;"
								data-max-width="25%">
								<xsl:call-template name="editable">
							...
							<text class="align-right" x="23%" y="13%" fill="var(--theme-sub-heading-color)"
								style="font-size:140%; font-family:var(--theme-body-font);">
								<xsl:call-template name="editable">
