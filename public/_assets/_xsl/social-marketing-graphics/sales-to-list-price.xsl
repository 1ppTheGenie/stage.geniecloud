...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="3.0" expand-text="yes">
  ...
  <xsl:template name="svg-body">
    ...
    <g style="transform:translate(2.5%,26%);">
      <g style="transform:translateX(2.5%)">
        <xsl:call-template name="lozenge">
          ...
          <xsl:with-param name="width">
            <xsl:call-template name="safe-number">
              <xsl:with-param name="value" select="//statistics/@averageListPriceForSold div $maxAllPrices" />
            </xsl:call-template>
          </xsl:with-param>
      ...
      <g style="transform:translate(2.5%, 14%)">
        <xsl:call-template name="lozenge">
          ...
          <xsl:with-param name="width">
            <xsl:call-template name="safe-number">
              <xsl:with-param name="value" select="//statistics/@averageSalePrice div $maxAllPrices" />
            </xsl:call-template>
          </xsl:with-param>
          <xsl:with-param name="value">
            <xsl:call-template name="format-price">
      ...
      <g style="transform:translate(2.5%, 31%)">
        <xsl:call-template name="lozenge">
          ...
          <xsl:with-param name="width">
            <xsl:call-template name="safe-number">
              <xsl:with-param name="value" select="//previous/@averageListPriceForSold div $maxAllPrices" />
            </xsl:call-template>
          </xsl:with-param>
          <xsl:with-param name="value">
            <xsl:call-template name="format-price">
              <xsl:with-param name="price" select="//previous/@averageListPriceForSold" />
            </xsl:call-template>
          </xsl:with-param>
        </xsl:call-template>
      </g>
      
      <g style="transform:translate(2.5%, 45%)">
        <xsl:call-template name="lozenge">
          <xsl:with-param name="fill-id" select="'2'" />
          <xsl:with-param name="caption-width" select="number(16)" />
          <xsl:with-param name="caption-align" select="'middle'" />
          ...
          <xsl:with-param name="width">
            <xsl:call-template name="safe-number">
              <xsl:with-param name="value" select="//previous/@averageSalePrice div $maxAllPrices" />
            </xsl:call-template>
          </xsl:with-param>
          <xsl:with-param name="value">
            <xsl:call-template name="format-price">
              <xsl:with-param name="price" select="//previous/@averageSalePrice" />
            </xsl:call-template>
          </xsl:with-param>
        </xsl:call-template>
      </g>
    ...
    <g style="transform: translate(75%, 28%);">
      <polyline style="fill:#fff; filter: drop-shadow(2px 3px 9px var(--theme-body-color));" points="-20,55 0,40 0,0 250,0 250,110 0,110 0,70"></polyline>
      ...
      <text x="10%" y="12.5%" class="center" fill="#000" style="font-size:350%;">
        <xsl:variable name="avgSalePrice">
          <xsl:call-template name="safe-number">
            <xsl:with-param name="value" select="//statistics/@averageSalePrice" />
          </xsl:call-template>
        </xsl:variable>
        <xsl:variable name="avgListPrice">
          <xsl:call-template name="safe-number">
            <xsl:with-param name="value" select="//statistics/@averageListPriceForSold" />
          </xsl:call-template>
        </xsl:variable>
        <xsl:value-of select="format-number($avgSalePrice div $avgListPrice, '#.00%')" />
      </text>
      <text x="20%" y="23%" class="align-right" style="font-size:125%; line-height:120%; dominant-baseline: alphabetic;">
    ...
    <g style="transform: translate(75%, 62%);">
      ...
      <text x="10%" y="12.5%" class="center" fill="#000" style="font-size:350%;">
        <xsl:variable name="prevAvgSalePrice">
          <xsl:call-template name="safe-number">
            <xsl:with-param name="value" select="//previous/@averageSalePrice" />
          </xsl:call-template>
        </xsl:variable>
        <xsl:variable name="prevAvgListPrice">
          <xsl:call-template name="safe-number">
            <xsl:with-param name="value" select="//previous/@averageListPriceForSold" />
          </xsl:call-template>
        </xsl:variable>
        <xsl:value-of select="format-number($prevAvgSalePrice div $prevAvgListPrice, '#.00%')" />
      </text>
      <text x="20%" y="23%" class="align-right" style="font-size:125%;dominant-baseline: alphabetic;">
        <tspan>Average Days on Market </tspan>
        <tspan dx="0.5%" class="bold">
