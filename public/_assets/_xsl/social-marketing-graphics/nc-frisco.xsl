...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="3.0" expand-text="yes">
	...
	<xsl:template name="svg-body">
		...
		<rect xmlns="" x="0%" y="0%" width="100%" height="16%" fill="var(--theme-sub-heading-color)"></rect>
		<g>
			<text x="50%" y="5.5%" class="upper super-bold center" font-family="var(--theme-heading-font)" font-size="260%" data-max-width="94%" fill="var(--theme-body-background)">
				<xsl:call-template name="editable">
					<xsl:with-param name="id" select="'areaname'" />
					<xsl:with-param name="default" select="concat(//area/name, ' ' , $propertyType)" />
				</xsl:call-template>
			</text>
			<text x="50%" y="12.5%" class="upper center" font-family="var(--theme-heading-font)" font-size="250%" font-weight="300" fill="var(--theme-body-background)">
				<xsl:call-template name="editable">
		...
		<g xmlns="" style="transform: translate(69%, -17%);">
			...
			<text class="upper bold" fill="var(--theme-body-color)" style="font-family:var(--theme-body-font); font-size:100%;">
		...
		<g style="transform: translate(80%, 20%);">
			<text x="6%" y="38%" width="26%" height="8%" fill="var(--theme-body-color)" class="center" style="font-family:var(--theme-body-font);">
		...
		<rect xmlns="" x="0%" y="79%" width="100%" height="12%" fill="var(--theme-body-color)"/>
		...
		<text x="70%" y="82%" font-size="130%" font-family="var(--theme-body-font)" class="upper align-right" fill="var(--theme-body-background)" data-max-width="40%">
			<xsl:call-template name="editable">
				<xsl:with-param name="id" select="'agentname'" />
				<xsl:with-param name="default" select="concat( //agent[1]/firstName, '&#160;', //agent[1]/lastName )" />
			</xsl:call-template>
		</text>
		<text x="70%" y="86%" font-size="130%" fill="var(--theme-body-background)" font-family="var(--theme-body-font)" class="bold align-right">
			<xsl:call-template name="editable">
				<xsl:with-param name="id" select="'agentmobile'" />
				<xsl:with-param name="default" select="//agent[1]/mobile" />
			</xsl:call-template>
		</text>
		<text x="70%" y="89.5%" fill="var(--theme-body-background)" fill-opacity="50%" font-size="50%" font-family="var(--theme-sub-heading-font)" class="align-right" data-max-width="30%">
			<xsl:call-template name="editable">
		...
		<rect xmlns="" x="0%" y="92%" width="100%" height="8%" fill="var(--theme-body-color)" fill-opacity="0.8"/>
		<text x="50%" y="96.5%" font-size="120%" font-family="var(--theme-body-font)" fill="var(--theme-body-background)" class="upper bold center" data-max-width="65%">
			<xsl:call-template name="editable">
				<xsl:with-param name="id" select="'areaname '" />
