	...
	<xsl:template name="svg-body">
	 <style>
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<image x="46%" y="87.4%" height="7%" width="26.5%" id="logo" preserveAspectRatio="xMinYMin meet">
					<xsl:attribute name="href">
						<xsl:choose>
							<xsl:when test="//output/@themeHue = 'light'">
								<xsl:value-of select="$personalLogo" />
				...
				<image x="75%" y="87.4%" height="6%" width="22%" id="logo" preserveAspectRatio="xMaxYMin meet">
					<xsl:attribute name="href">
						<xsl:choose>
							<xsl:when test="//output/@themeHue = 'light'">
								<xsl:value-of select="$companyLogo" />
