<?xml version="1.0"?>
<!--
	Asset Name: Andy Tse Single Listing 3
...
<xsl:with-param name="default" select="'Bath &#124; '" />
		...
		<g style="transform:translate(67%, 68%);">
			...
			<text class="open-sans upper bold align-right" x="2%" y="6.5%" fill="#fdb913" style="font-size:150%;line-height:120%;">
				<xsl:call-template name="editable">
		...
		<text x="50%" y="95.5%" style="font-size:150%;" fill="#2c3740" class="open-sans upper center bold" data-max-width="95%">
			<xsl:call-template name="editable">
				...
				<xsl:with-param name="default" select="concat(//area/name, '’S #1 Agent' )" />
			</xsl:call-template>
		</text>
	</xsl:template>
</xsl:stylesheet>
