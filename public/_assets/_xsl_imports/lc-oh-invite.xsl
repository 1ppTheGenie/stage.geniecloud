    ...
    <text x="50%" y="0%" class="center bold" font-family="var(--theme-sub-heading-font)" fill="var(--theme-body-color)" text-anchor="middle" font-size="320%" style="dominant-baseline: middle; letter-spacing:4px;">
        <tspan>
            <xsl:call-template name="editable">
                ...
                <xsl:with-param name="default">
                    ...
                    <xsl:call-template name="ordinal-suffix">
                        <xsl:with-param name="date" select="number(//openHouse/session[1]/@date)" />
                    </xsl:call-template>
                    <xsl:text> - </xsl:text>
                </xsl:with-param>
            </xsl:call-template>
        ...
        <tspan style="text-transform: lowercase">
            <xsl:call-template name="editable">
                <xsl:with-param name="id" select="'LC-OH-INVITE-01'" />
                <xsl:with-param name="default">
                    <xsl:value-of select="concat(//openHouse/session[1]/@starts, ' to ', //openHouse/session[1]/@ends)" />
                </xsl:with-param>
            </xsl:call-template>
        </tspan>
    </text>
