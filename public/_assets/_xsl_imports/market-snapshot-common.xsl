...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template name="market-snapshot-common">
		...
		<text x="3%" y="39%" font-size="140%" class="bold">
			<tspan>
				<xsl:text>Highest home price of </xsl:text>
		...
		<text x="3%" y="56%" font-size="140%" class="bold">
			<tspan>
				<xsl:text>Average home price of </xsl:text>
				<xsl:call-template name="property-type-caption" />
			</tspan>
		</text>
		...
		<g id="avg-sale-up-down-rate">
			<use class="bold" x="15.6%" y="76%" width="25%" height="2.2%" fill="transparent" stroke="var(--theme-heading-color)" stroke-width="15">
			...
			<text x="29%" y="76%" class="normal" fill="var(--theme-heading-color)">
				<tspan>
					<xsl:variable name="since">
						<xsl:value-of select="genie:format-date( //output/@sinceDatee, '[MNn] [Y0001]')" />
					</xsl:variable>
		...
		<text x="3%" y="71%" font-size="140%" class="bold">
			<xsl:text>Median home price of </xsl:text>
			<xsl:call-template name="property-type-caption" />
		</text>
