...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="3.0" expand-text="yes">
	...
	<xsl:template name="single-listing-common-05">
		<style>
			<xsl:value-of select="'
		...
		<xsl:choose>
			<xsl:when test="number(//single/statusTypeID) = 2">
				<image x="90%" y="0" width="5.5%" height="12.5%" preserveAspectRatio="xMidYMid meet">
					<xsl:attribute name="href">
						<xsl:value-of select="concat( //output/@siteUrl, '_assets/_img/sold-tag.png')" />
					</xsl:attribute>
				</image>
			</xsl:when>
			...
			<xsl:otherwise>
				<image x="90%" y="0" width="5.5%" height="12.5%" preserveAspectRatio="xMidYMid meet">
					<xsl:attribute name="href">
						<xsl:value-of select="concat( //output/@siteUrl, '_assets/_img/pending-tag.png')" />
					</xsl:attribute>
				</image>
			</xsl:otherwise>
		</xsl:choose>
		...
		<g style="transform:translate(1%,2%)">
			<text x="1%" y="0" class="super-large bold" fill="var(--theme-sub-heading-color)">
				<tspan class="upper narrow">
					<xsl:choose>
						<xsl:when test="(//single/statusTypeID=3) or (//single/statusTypeID=4) or (//single/statusTypeID=12)">
