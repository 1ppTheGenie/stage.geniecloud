	...
	<xsl:template name="map-listings-table-header">
		...
		<style>
			...
			<xsl:value-of select="'
				...
				#genie-map{border-radius: 0;} table.listings{font-size:90%;} #genie-wrapper[viewBox=&quot;0 0 555 555&quot;] table.listings{font-size:70%;}

				table.listings td + td {
					position: relative;
				}
...
background-color: var(--new-blue);
	...
	<xsl:template name="map-listings-table">
		...
		<style>
			<xsl:value-of select="concat( ':root { --color-scheme:	var(', $colorScheme, ');--theme-heading-color: var(--color-scheme);--theme-sub-heading-color: var(--color-scheme);}' )" />

			<xsl:value-of select="' #genie-wrapper[viewBox=&quot;0 0 555 555&quot;] .hide-small {
					display: none;
				}
...
'" />
		...
		<g style="transform: translateY(-5%);">
			...
			<text x="97%" text-anchor="end" font-family="var(--theme-sub-heading-font)">
				<xsl:attribute name="y">
					<xsl:choose>
						<xsl:when test="$isFBAd='true'">
							<xsl:value-of select="'19%'" />
		...
		<g>
			...
			<rect width="95%" x="0" stroke-width="2" stroke="var(--color-scheme)" fill-opacity="0" height="67%">

			</rect>
			<g style="clip-path: url(#mapClipPath); transform: translate(1.8px, 2px);">
				<foreignObject>
					<xsl:attribute name="width">
						<xsl:value-of select="$mapWidth" />
					</xsl:attribute>
