		...
		<div className="flex-row" style={props.style}>
			...
			<div class="w-66">
                <div class="listDetail">
                    <table id="listing-table" class="active-table" style=" font-size: 80%;">
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        <Show when={settings.marketstatus === "sold"}>
                            <For each={props.listings}>
                                {l => (
                                    <tr key={`tr${l.streetNumber}:${l.latitude}:${l.longitude}`}>
                                        <td>{address(l)}</td>
                                        <td>{l.bedrooms}</td>
                                        ...
                                        <td>{l.soldDate && format(Date.parse(l.soldDate), "M-d-y")}</td>
                            ...
                            <For each={props.listings}>
                                {l => (
                                    <tr key={`tr${l.streetNumber}:${l.latitude}:${l.longitude}`}>
                                        ...
                                        <td>{l.sqft > 0 ? currency(l.priceHigh / l.sqft) : "-"}</td>
                                        <td>{l.daysOnMarket > 0 ? l.daysOnMarket + ' Days' : "Today!"}</td>
                                    </tr>
                                )}
                            </For>
                        </Show>
