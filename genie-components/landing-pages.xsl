...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="3.0" expand-text="yes">
	...
	<xsl:include href="printers-marks.xsl"/>
	...
	<xsl:template match="/">
		<html lang="en">
			<xsl:attribute name="class">
				<xsl:value-of select="concat( 'genie-landing-page ', //output/@stylesheet, ' plugin-v', //output/@pluginVersion,' asset-v', //output/@assetVersion )" />
			</xsl:attribute>
			...
			<script>
				...
				<xsl:value-of select="concat( 'window.ghub = { SINGLE:  0, CONDO: 1, site_url: `', //output/@siteUrl, '`, api_url: `', //output/@siteUrl, '`, mapStyle: `satellite-v9`, google_key: `', //output/@googleKey, '`, mapbox_key: `', //output/@mapboxKey, '` };')" />
			</script>
						...
						<div class="col-md-4 col-sm-4 ">
							<img alt="1parkplace">
								<xsl:attribute name="src">
									<xsl:value-of select="concat( //output/@siteUrl, '/assets/_img/1pp-logo.png')" />
								</xsl:attribute>
							</img>
...
-->
	...
	<xsl:template name="css-links">
		...
		<xsl:if test="$primaryCSS!=''">
			<link rel="stylesheet" type="text/css">
				<xsl:attribute name="href">
					<xsl:value-of select="concat( //output/@siteUrl, '_assets/landing-pages/css/', $primaryCSS, '.css')" />
				</xsl:attribute>
			</link>
		</xsl:if>
		...
		<xsl:if test="$secondaryCSS!=''">
			<link rel="stylesheet" type="text/css">
				<xsl:attribute name="href">
					<xsl:value-of select="concat( //output/@siteUrl, '_assets/landing-pages/css/', $secondaryCSS, '.css')" />
				</xsl:attribute>
			</link>
		</xsl:if>
	...
	<xsl:template name="standard-header">
		...
		<xsl:variable name="buildVersion" select="'2.0.0'" />
		...
		<head>
			...
			<link rel="stylesheet">
				<xsl:attribute name="href">
					<xsl:value-of select="concat( //output/@siteUrl, '_assets/landing-pages/dist/', $buildVersion, '/assets/index.css' )" />
				</xsl:attribute>
			</link>
		</head>
	</xsl:template>
	...
	<xsl:template name="market-comment">
		<xsl:variable name="totalSold">
			<xsl:value-of select="//areas/area[1]/statistics/@soldPropertyTypeCount"/>
		</xsl:variable>
		<xsl:variable name="totalSoldLastPeriod">
			<xsl:value-of select="//areas/area[1]/statistics/previous/@totalSold"/>
		</xsl:variable>
				...
				<div class="modal-content">
					<div class="modal-body">
						<div class="modal-background-img background">
							...
							<h2 class="widget editable text-center heading-font subtitle-color">
		...
		<div id="flyer-popup" class="popup-dialog flyer-popup">
			<div class="modal-dialog modal-dialog-centered" role="document">
				...
				<div class="modal-content">
					<div class="modal-body">
						<div class="modal-background-img background">
							<h1 class="widget editable text-center heading-font heading-color">                            
                                WELCOME!                            
							</h1>
							<image preserveAspectRatio="xMidYMid meet" style="margin:10px auto; display:block; position:relative; max-width:100%;">
								<xsl:attribute name="src">
									<xsl:value-of select="concat( //output/@siteUrl, '_assets/_img/welcome-modal.jpg' )" />
								</xsl:attribute>
							</image>
	...
	<xsl:template name="add-genie-embed">
		...
		<div class="genie-embed">
			...
			<xsl:attribute name="data-genie-renderid">
				<xsl:value-of select="//output/@renderID" />
			</xsl:attribute>
	...
	<xsl:template name="gallery">
		<xsl:if test="count(//single/images/image)&gt;1">
			...
			<script>
				<xsl:text>window.gg.galleryImages=[</xsl:text>
				<xsl:for-each select="//single/images/image">
					<xsl:value-of select="concat( &quot;'&quot;, @src, &quot;', &quot; )" />
				</xsl:for-each>
				<xsl:text>];</xsl:text>
				<xsl:text>document.addEventListener(`genie-landing-loaded`, () => window.gg.galleryButton( document.querySelector('.property-image-show-btn button'), document.getElementById('property-gallery') ) );</xsl:text>
			</script>
		</xsl:if>
	</xsl:template>
</xsl:stylesheet>