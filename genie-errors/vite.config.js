...
const __BASE__ =
	process.env.NODE_ENV === "production" ? "/genie-tools/error-viewer/" : "";
	...
	return {
		...
		transformIndexHtml(html) {
			["js", "css"].forEach(suffix => {
				html = html.replace(
					`/genie-tools/error-viewer/index.${suffix}`,
					`/genie-tools/error-viewer/index.${suffix}?v=${__BUILD_VERSION__}`
				);
			});
...
export default defineConfig({
	...
	base: __BASE__,
	build: {
		outDir: `../public/genie-tools/error-viewer/`,
		sourcemap: true,
		minify: false,
		target: "esnext",
